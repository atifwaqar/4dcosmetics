...
const CartDrawer = (() => {
  ...
  function build() {
    ...
    panel.addEventListener('click', e => {
      ...
      else if (t.closest('[data-remove]')) { handleRemove(t.closest('[data-remove]')); }
    });
        ...
        <div class="cart-line" data-id="${it.id}">
          ...
          <div>
            ...
            <div class="cart-line__qty">
              <button class="btn-reset" data-dec="${it.id}">−</button>
              <input type="number" min="1" value="${it.qty}" data-qty="${it.id}">
              <button class="btn-reset" data-inc="${it.id}">+</button>
              <button class="btn-reset text-danger" data-remove="${it.id}">Remove</button>
            </div>
          </div>
